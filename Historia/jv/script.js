document.addEventListener('DOMContentLoaded', () => {
  ...
  // Llamada al 911 (solo si el botón existe)
  if (btnLlamada911) {
  ...
  document.addEventListener('click', (event) => {
    ...
    if (menu && menu.classList.contains('active') && menuContainer && !menuContainer.contains(event.target)) {
  ...
  // Diccionario de departamentos y municipios con sus URL (se mantiene igual)
  const dpiLocations = {
    "Atlantida": [
    {
      name: "La Ceiba",
 ...
 "Gracias a Dios": [
  ...
  if (selectDepartamento) {
    selectDepartamento.addEventListener('change', () => {
      ...
      if (!deptoSeleccionado || !dpiLocations[deptoSeleccionado]) {
        if (selectMunicipio) {
          selectMunicipio.innerHTML = '<option value="">-- Primero elige un departamento --</option>';
          selectMunicipio.disabled = true;
        }
      ...
      dpiLocations[deptoSeleccionado].forEach(mun => {
        const option = document.createElement('option');
  ...
  /************************************************
   * 4) BARRA FLOTANTE DE REDES (Cerrar con la X)
   ***********************************************/
